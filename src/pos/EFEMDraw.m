...
classdef EFEMDraw < handle
    ...
    methods (Access = public) % Access from methods in subclasses
        ...
        function elements(draw)
            for el = 1:draw.model.nelem
                ...
                for i = 1:length(res)
                    faces      = res(i).faces;
                    vertices   = res(i).vertices;
                    vertexData = res(i).vertexData;
                    patch('Faces',faces,...
                        'Vertices',vertices,...
                        'FaceVertexCData',vertexData,...
                        'FaceColor',res(i).faceColor,...
                        'LineWidth',res(i).edgesThickness,...
                        'Marker',res(i).markerType,...
                        'MarkerFaceColor',res(i).markerFaceColor); 
                end
            end
            colormap("jet")
        end
    ...
    methods        
        ...
        function bbox = EFEMBoundBox(draw)
            ...
            bbox(1) = min(draw.model.NODE(:,1)) - tolx;
            bbox(2) = min(draw.model.NODE(:,2)) - toly;
            bbox(3) = max(draw.model.NODE(:,1)) + tolx;
            bbox(4) = max(draw.model.NODE(:,2)) + toly;
        end
